                                ...
                                <div class="InnerTableContainer">
                                        <table width="100%">
                                                <tr>
                                                        ...
                                                        <td><div style="position: absolute; top: 35px; right: 6px;"><img class="PVPTypeIcon" src="images/option_server_pvp_type_retro.gif" alt="Server PVP Type"></div>O'.($config['status']['serverStatus_online'] == 1 ? 'n' : 'ff').'line</td>
                                                </tr>
                                                <tr>
...
if(file_exists($f) && filemtime($f) > (time() - $cache_sec)) {
        ...
        $cp = '';
        $n = 0;
        $q = 'SELECT accounts.flag, players.name, players.vocation, players.level, players.skull, players.looktype, players.lookaddons, players.lookhead, players.lookbody, players.looklegs, players.lookfeet FROM accounts, players, players_online WHERE players.id = players_online.player_id AND accounts.id = players.account_id';
        ...
        if(in_array($order, array('vocation_asc','vocation_desc'))) {
                ...
                foreach($SQL->query($q)->fetchAll() as $p)
                        $a[] = array($p['name'], $p['level'], $p['vocation'], array($p['looktype'], $p['lookaddons'], $p['lookhead'], $p['lookbody'], $p['looklegs'], $p['lookfeet']));
                function cmp($a, $b) {
                        $r = strcmp($a[2], $b[2]);
                ...
                foreach($a as $p) {
                        $n++;
                        $cp .= '<tr class="'.(is_int($n/2)?'Odd':'Even').'" style="text-align:right"><td><div style="position: relative; width: 32px; height: 32px;"><div style="background-image: url(\'outfit.php?id='.$p[3][0].'&addons='.$p[3][1].'&head='.$p[3][2].'&body='.$p[3][3].'&legs='.$p[3][4].'&feet='.$p[3][5].'\'); position: absolute; width: 64px; height: 80px; background-position: bottom right; background-repeat: no-repeat; right: -10px; bottom: 0px;"></div></div></td><td style="width:70%;text-align:left"><a href="?subtopic=characters&name='.urlencode($p[0]).'">'.$p[0].'</a></td><td style="width:10%">'.$p[1].'</td><td style="width:20%">'.htmlspecialchars($vocation_name[$p[2]]).'</td></tr>';
                ...
                foreach($SQL->query($q)->fetchAll() as $p) {
                        ...
                        $cp .= '<td><div style="position: relative; width: 32px; height: 32px;"><div style="background-image: url(\'outfit.php?id='.$p['looktype'].'&addons='.$p['lookaddons'].'&head='.$p['lookhead'].'&body='.$p['lookbody'].'&legs='.$p['looklegs'].'&feet='.$p['lookfeet'].'\'); position: absolute; width: 64px; height: 80px; background-position: bottom right; background-repeat: no-repeat; right: -10px; bottom: 0px;"></div></div></td>';
                        if($order == 'name_asc') {
                        ...
                        $cp .= '<td style="width:70%;text-align:left"><a href="?subtopic=characters&name='.urlencode($p['name']).'">'.$p['name'].'</a></td><td style="width: 10%">'.$p['level'].'</td><td style="width:20%">'.htmlspecialchars($vocation_name[$p['vocation']]).'</td></tr>';
                }
